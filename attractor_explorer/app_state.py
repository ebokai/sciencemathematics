...
class AppState:
    ...
    def handle_mousedown(self):
        ...
        self.clicked, self.parameters, self.active_slider_id = slider.intersect_slider(
            mouse_position, self.parameters, self.active_slider_id
        )
        if self.clicked:
            self.clear_and_draw()

    def update_active_slider(self, event):
        slider_delta = -1 * (event.key == pygame.K_UP) + 1 * (
    ...
    def update_parameter(self):
        ...
        self.parameters[self.active_slider_id] = np.clip(
            self.parameters[self.active_slider_id] + delta, -1, 1
        )
        self.clear_and_draw()
    ...
    def clear_and_draw(self):
        ...
        self.rasterization_entropy, self.fourier_transform_data = (
            functions.compute_grid_stats(normalized_x, normalized_y)
        )
        self.parameter_change = True
    ...
    def draw_fourier_spectrum(self):
        ...
        if self.rasterization_entropy > 0:
            ...
            pygame.draw.lines(
                self.surface_left, PIXEL_COLOR, False, fourier_line_spectrum
            )
